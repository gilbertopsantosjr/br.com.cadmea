...
import br.com.cadmea.comuns.orm.enums.Ordenacao;
 ...
 * pre-definidos.
 * 
 * @param <E>  Entidade a ser manipulado
 * @author Gilberto Santos
 * @version 1.0
 */
public abstract class DaoGenericoImp<T extends BaseEntidade, ID extends Serializable> implements DaoGenerico<T> {
	...
	public T find(String namedQuery, Map<String, Object> parameters, Result resultado) {
		T retorno = null;
		TypedQuery<T> query = obterPorNamedQuery(namedQuery, parameters);
		if (resultado.equals(Result.UNIQUE))
			retorno = (T) obterApenasUmaEntidade(query);
		return retorno;
	}
	...
	@SuppressWarnings("unchecked")
	private TypedQuery<T> obterPorNamedQuery(String namedQuery, Map<String, Object> parameters) {
	...
	public T find(Serializable identificador) {
		T obj = null;
		try {
			obj = (T) getEntityManager().find(getClazz(), identificador);
			if (obj == null)
				throw new RuntimeException("Not possible to find entity "); // substituir por chave da mensagem
	...
	private Collection<T> obter(Map<String, Object> params, Criteria criteria) throws RuntimeException {
	...
	public T find(String propriedade, Object valor, Result res) throws NoneExistException {
		Map<String, Object> params = new HashMap<String, Object>();
		params.put(propriedade, valor);
	...
	@Override
	public Collection<T> find(Map<String, Object> params) throws RuntimeException {
	...
	private Criteria consulta(Map<String, Object> params, Criteria criteria) {
		...
		for (String chave : chaves) {
			String propNome = (String) chave;
			Object valor = params.get(chave);
			if (valor instanceof Enum) {
				if (valor.getClass().isEnum() && !Util.isVazio((Enum<?>) valor))
					if (!(valor instanceof Ordenacao)) {
						criteria.add(Restrictions.eq(propNome, (Enum<?>) valor));
					}
			}
			if (valor instanceof Number) {
				criteria.add(Restrictions.eq(propNome, (Number) valor));
			}
			if (valor instanceof Date) {
				criteria.add(Restrictions.eq(propNome, (Date) valor));
			}
			if (valor instanceof CriteriaDataConstrutor) {
				CriteriaDataConstrutor criteriaDataConstrutor = (CriteriaDataConstrutor) valor;
				for (CriteriaData criteriaData : criteriaDataConstrutor.getCriterioData()) {
					...
					case IGUAL_OU_MENOR:
						...
						break;
					case ENTRE:
						criteria.add(Restrictions.between(propNome, criteriaData.getDataUm(), criteriaData.getDataDois()));
						break;
					default:
						break;
					}
			...
			if (valor instanceof String) {
				...
				if (stringIdentificavel.contains(PERCENT)) {
					...
					criteria.add(Restrictions.eq(propNome, (String) valor).ignoreCase());
			}

			if (valor instanceof Ordenacao) {
				Ordenacao ordenacao = (Ordenacao) valor;
				criteria.addOrder(ordenacao.equals(Ordenacao.ASC) ? Order.asc(propNome) : Order.desc(propNome));
			}
	...
	@SuppressWarnings("unchecked")
	protected Collection<T> obter(Criteria criteria, ResultTransformer resultTransformer) {
	...
	public T find(Map<String, Object> params, Result resultado) throws NoneExistException {
